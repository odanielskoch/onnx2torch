...
class OnnxScatterElements(nn.Module, OnnxToTorchModule):  # pylint: disable=missing-docstring
    def __init__(
        self,
        axis: int
    ):
    ...
    def forward(  # pylint: disable=missing-function-docstring
        ...
        assert len(data.shape) == len(indices.shape) # same rank
        assert updates.shape == indices.shape # same shape

        result = data.clone()
        result.scatter_(dim=self.axis, index=indices, src=updates)
        return result


@add_converter(operation_type='ScatterElements', version=11)
@add_converter(operation_type='ScatterElements', version=13)
def _(node: OnnxNode, graph: OnnxGraph) -> OperationConverterResult:  # pylint: disable=unused-argument
    node_attributes = node.attributes
    axis = node_attributes.get('axis', 0)

    return OperationConverterResult(
        torch_module=OnnxScatterElements(axis=axis),
